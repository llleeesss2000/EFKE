...
if command -v ss >/dev/null 2>&1 && ss -ltn | awk '{print $4}' | grep -q ":${USER_WEB_PORT}$"; then
  listener_pid="$(ss -ltnp 2>/dev/null | awk "/:${USER_WEB_PORT} / {print \\$0}" | sed -n 's/.*pid=\\([0-9]*\\).*/\\1/p' | head -n 1)"
